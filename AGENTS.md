...
10a. **DPU⇄NIC mode switch applies via `mlxfwreset`, NOT a warm reboot, and mlxconfig output must be read by *column*.** `INTERNAL_CPU_MODEL` (EMBEDDED_CPU(1)=DPU / SEPARATED_HOST(0)=NIC) is the only knob to touch — leave the ownership sub-knobs alone. Two traps, both live-proven on BF3 (`tky-bnk-dpu-host-2` @ `0d:00.0`):
   - **A warm host reboot does NOT apply a staged change** — the BlueField keeps its running firmware config across it, so `Current` stays put while `Next Boot` shows the staged value. Only a firmware reset (`mlxfwreset -d <pci> -y reset`, default level 3) — or a cold power cycle — flips `Current`. `provision.FWResetCmd` / `provision set-mode` do this; never "reboot to apply".
   - **`mlxconfig -e q` prints three columns (Default | Current | Next Boot).** A naive `strings.Contains(out, "EMBEDDED_CPU")` can match the *Default* column and misreport a NIC-mode card as DPU. Read the **Current** column (the `*` prefix marks Current≠Default). `provision.ReadCPUMode` parses this correctly. **Known follow-up:** `internal/provision/readiness.go` on `feat/forge-embedded-agent` still uses the naive substring match + reruns the same mlxconfig read — consolidate it onto `ReadCPUMode`. Note `provision set-mode` is a standalone Day-2 subcommand outside the phase pipeline (gated with `--yolo` + `--confirm-cluster`; `--stage-only` keeps `--confirm-cluster` as a typo guard).
