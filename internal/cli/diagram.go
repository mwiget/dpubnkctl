...
func RenderVLANsASCII(p *poc.PoC) string {
	...
	// Iface naming: host VLAN sub-interfaces and DPU OVS ports both
	// derive their name from Role+Tag (via PortName()), so e.g. VLAN 40
	// with role=external is the linux iface "external40" on the host
	// AND the OVS port "external40" on the DPU AND the F5SPKVlan name
	// "external40" that TMM binds. Showing the same iface column for
	// every row in a VLAN block is intentional — it documents that
	// uniform naming.
	type member struct{ label, iface, ip string }
	...
	for _, h := range p.Hosts {
		if h.DataPlane != nil {
			for _, v := range h.DataPlane.VLANs {
				byTag[v.Tag] = append(byTag[v.Tag], member{label: h.Name, iface: v.PortName(), ip: lastOctetOf(v.IP)})
			}
		}
		for _, d := range h.DPUs {
			label := d.Hostname
			...
			for _, v := range d.VLANs {
				byTag[v.Tag] = append(byTag[v.Tag], member{label: label, iface: v.PortName(), ip: lastOctetOf(v.IP)})
			}
		}
	}
	add := func(role, ip string) {
		...
		for _, v := range p.Network.VLANs {
			if v.Name == role {
				// TMM-side F5SPKVlan name follows the same Role+Tag
				// convention; auto-generated by deploy.RenderF5SPKVlans.
				iface := fmt.Sprintf("%s%d", v.Name, v.ID)
				byTag[v.ID] = append(byTag[v.ID], member{label: "TMM self-IP", iface: iface, ip: lastOctetOf(ip)})
			}
		}
	}
	...
	for _, t := range tags {
		...
		labelW, ifaceW := len("Node"), len("Iface")
		for _, m := range byTag[t] {
			if n := len(m.label); n > labelW {
				labelW = n
			}
			if n := len(m.iface); n > ifaceW {
				ifaceW = n
			}
		}
		fmt.Fprintf(&b, "    %-*s    %-*s    %s\n", labelW, "Node", ifaceW, "Iface", "Address")
		fmt.Fprintf(&b, "    %s    %s    %s\n",
			strings.Repeat("-", labelW), strings.Repeat("-", ifaceW), strings.Repeat("-", len("Address")))
		for _, m := range byTag[t] {
			fmt.Fprintf(&b, "    %-*s    %-*s    .%s\n", labelW, m.label, ifaceW, m.iface, m.ip)
		}
		fmt.Fprintln(&b)
	}
	return b.String()
