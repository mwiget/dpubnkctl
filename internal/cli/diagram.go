...
func RenderVLANsASCII(p *poc.PoC) string {
	...
	// Host VLAN sub-interfaces, DPU OVS ports, and the F5SPKVlan that
	// TMM binds all share the same Role+Tag iface name. Show that name
	// once in the table title (no per-row repetition) and put the full
	// IP in the Address column.
	//
	// TMM self-IPs are intentionally NOT synthesized from
	// bnk.external_selfip / bnk.internal_selfip: that's a single VIP
	// field, but each DPU runs its own TMM with its own listener — so
	// showing one value would be misleading. The per-DPU IPs already
	// appear under each DPU row.
	type member struct{ label, ip string }
	byTag := map[int][]member{}
	subnetByTag := map[int]string{}
	roleByTag := map[int]string{}
	ifaceByTag := map[int]string{}
	for _, v := range p.Network.VLANs {
		subnetByTag[v.ID] = v.Subnet
		roleByTag[v.ID] = v.Name
		ifaceByTag[v.ID] = fmt.Sprintf("%s%d", v.Name, v.ID)
	}
	for _, h := range p.Hosts {
		if h.DataPlane != nil {
			for _, v := range h.DataPlane.VLANs {
				byTag[v.Tag] = append(byTag[v.Tag], member{label: h.Name, ip: stripCIDR(v.IP)})
			}
		}
		for _, d := range h.DPUs {
			label := d.Hostname
			...
			for _, v := range d.VLANs {
				byTag[v.Tag] = append(byTag[v.Tag], member{label: label, ip: stripCIDR(v.IP)})
			}
		}
	}
	...
	for _, t := range tags {
		iface := ifaceByTag[t]
		if iface == "" {
			// Network.VLANs may not declare this tag (DPU/host vlan added
			// later) — derive from the first member's role if possible.
			iface = fmt.Sprintf("vlan%d", t)
		}
		header := fmt.Sprintf("%s VLAN %d (%s) — %s", roleByTag[t], t, iface, subnetByTag[t])
		...
		fmt.Fprintf(&b, "    %-*s    %s\n", labelW, "Node", "Address")
		fmt.Fprintf(&b, "    %s    %s\n",
			strings.Repeat("-", labelW), strings.Repeat("-", len("Address")))
		for _, m := range byTag[t] {
			fmt.Fprintf(&b, "    %-*s    %s\n", labelW, m.label, m.ip)
		}
		fmt.Fprintln(&b)
